...
class BlackboardEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, questions: list = None):
        obs, infos = self.envs.reset(questions=questions)
    ...
    def step(self, text_actions: List[str]):
        ...
        for i, info in enumerate(infos):
            info['is_action_valid'] = to_numpy(valids[i])
            info['input_token_num'] = to_numpy(info['input_token_num'])
            info['output_token_num'] = to_numpy(info['output_token_num'])#######################################
            #将action_count_dict转为numpy
            # action_count_dict_numpy = {}
            # for key, value in info['action_count_dict'].items():
            #     action_count_dict_numpy[key] = to_numpy(value)
            # info['action_count_dict'] = action_count_dict_numpy

        next_observations = {'text': full_text_obs, 'image': None, 'anchor': text_obs}
        #print('observation:',next_observations['text'])
        rewards = to_numpy(rewards)
        dones = to_numpy(dones)
        ...
        if init:
            ...
            for i in range(len(text_obs)):
                obs = BLACKBOARD_TEMPLATE.format(
                    ...
                    current_step=len(self.memory[i]) + 1,
                    previous_action=infos[i]['previous_action'],
                    previous_output=infos[i]['previous_output']
                )
                postprocess_text_obs.append(obs)
...
def make_envs(config):
    ...
    if "gym_cards" in config.env.env_name.lower():
        ...
        return envs, val_envs
    elif "blackboard" in config.env.env_name.lower():
        ...
        _envs = build_blackboard_envs(seed=config.env.seed, config=dataset_config, env_num=config.data.train_batch_size, group_n=group_n, is_train=True)
        ...
        envs = BlackboardEnvironmentManager(_envs, projection_f, config)
