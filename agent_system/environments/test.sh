...
val_data_size=16
group_size=8
dataset_name='MMLU-Pro-subset'

# We only use data preparation to indicate the modality and the data size.

python3 -m agent_system.environments.test \
    ...
    data.max_prompt_length=9092 \
    data.max_response_length=1024 \
    data.filter_overlong_prompts=True \
    data.truncation='error' \
    data.return_raw_chat=True \
    actor_rollout_ref.model.path=$HOME/models/Qwen3-4B-Instruct \
    ...
    trainer.experiment_name='grpo_qwen3-4b' \
