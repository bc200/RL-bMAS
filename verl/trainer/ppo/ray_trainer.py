...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        #print("RayPPOTrainer config:", OmegaConf.to_container(config, resolve=True))
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "answer" in test_batch.non_tensor_batch:
                #print('have answer')
                #print(f"answer: {test_batch.non_tensor_batch['answer']}")
                non_tensor_batch_keys_to_pop.append("answer")
            if "answer_type" in test_batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("answer_type")
            if "decision_prompt" in test_batch.non_tensor_batch:
                non_tensor_batch_keys_to_pop.append("decision_prompt")
            #print(f"non_tensor_batch keys: {test_batch.non_tensor_batch.keys()}")
            #print("non_tensor_batch_keys_to_pop: ", non_tensor_batch_keys_to_pop)
            ...
            print('gen_batch size:', len(test_gen_batch))
            print('obs size', self.val_envs.get_num_envs())
            ...
            #validate token
            batch_prompt_token =  test_batch.non_tensor_batch["total_input_token_num"][0].item()
            batch_completion_token = test_batch.non_tensor_batch["total_output_token_num"][0].item()
            total_prompt_tokens += batch_prompt_token
            total_completion_tokens += batch_completion_token

            #validate action history
            if 'episode_action_history_dict' in test_batch.non_tensor_batch:
                batch_action_history_dict = test_batch.non_tensor_batch['episode_action_history_dict'][0]
                for k, v in batch_action_history_dict.items():
                    if k not in total_action_history_dict:
                        total_action_history_dict[k] = 0
                    total_action_history_dict[k] += v
        ...
        print('val_action_history_dict', total_action_history_dict)
        ...
        metric_dict['val/total_prompt_tokens'] = total_prompt_tokens
        metric_dict['val/total_completion_tokens'] = total_completion_tokens
    ...
    def init_workers(self):
        ...
        if self.hybrid_engine:
            ...
            print("actor_rollout_cls: ", actor_rollout_cls)
    ...
    def fit(self):
        ...
        if self.val_reward_fn is not None and self.config.trainer.get("val_before_train", True):
            ...
            if self.config.trainer.get("val_only", True):################################################
                return
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "answer" in batch.non_tensor_batch:
                    ...
                    #print(f"answer: {test_batch.non_tensor_batch['answer']}")
                    non_tensor_batch_keys_to_pop.append("answer")
                if "answer_type" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("answer_type")
                if "decision_prompt" in batch.non_tensor_batch:
                    non_tensor_batch_keys_to_pop.append("decision_prompt")
                gen_batch = batch.pop(
                    batch_keys=batch_keys_to_pop,
                    non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
